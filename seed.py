from app import create_app # Adjust import if your Flask app instance is named differently
...
def seed_database():
    ...
    with app.app_context():
        print("WARNING: This will drop all existing data. Press Ctrl+C to cancel within 3 seconds.")
        # import time as t; t.sleep(3) # Uncomment if you want a safety delay
        ...
        print("--- Admin Created (admin@arogya.com / admin123) ---")

        # 4. Create Doctors (User + Doctor Profile)
        # List of dummy doctor data
        doctor_data = [
        ...
        for i, (name, gender, age) in enumerate(patient_data):
            ...
            pat = Patient(
                ...
                dob=date(2024 - age, 1, 1), # Approx DOB based on age
                age=age,
                user_id=user.user_id
            )
            db.session.add(pat)
        ...
        # Add slots for Today and Tomorrow
        availabilities = []
        today = date.today()
        
        for doc in doctors_objs:
            # Slot 1: Today 10:00 - 11:00
        ...
        # A. Completed Appointment (History)
